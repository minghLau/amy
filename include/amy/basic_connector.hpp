...
namespace amy {
    ...
    public:
        ...
        /// functionalities.
        using service_type = MySQLService ;
        ...
        std::string error_message(AMY_SYSTEM_NS::error_code const& ec) {
            return this->get_service().error_message(this->get_implementation(), ec);
        }
        ...
        void close() {
            return this->get_service().close(this->get_implementation());
        }
        
        template<typename Option>
        void set_option(Option const& option) {
            AMY_SYSTEM_NS::error_code ec;
            detail::throw_error(set_option(this->get_implementation(), option, ec),
        ...
        AMY_SYSTEM_NS::error_code connect(Endpoint const& endpoint,
                                          ...
                                          AMY_SYSTEM_NS::error_code& ec)
        {
            return this->get_service().connect(this->get_implementation(),
                           ...
                           ConnectHandler handler)
        {
            return this->get_service().async_connect(
                                                     this->get_implementation(), endpoint, auth, database, flags, handler);
        }
        
        void query(std::string const& stmt) {
            AMY_SYSTEM_NS::error_code ec;
            detail::throw_error(query(stmt, ec), &(this->get_implementation().mysql));
        }
        
        AMY_SYSTEM_NS::error_code query(std::string const& stmt,
                                        AMY_SYSTEM_NS::error_code& ec)
        {
            return this->service.query(this->get_implementation(), stmt, ec);
        }
        
        template<typename QueryHandler>
        void async_query(std::string const& stmt, QueryHandler handler) {
            this->get_service().async_query(this->get_implementation(), stmt, handler);
        }
        ...
        void async_store_result(StoreResultHandler handler) {
            this->get_service().async_store_result(this->get_implementation(), handler);
        }
        ...
        AMY_SYSTEM_NS::error_code rollback(AMY_SYSTEM_NS::error_code& ec) {
            this->service.rollback(this->get_implementation(), ec);
            return ec;
        }
        
        uint64_t affected_rows() {
            return this->service.affected_rows(this->get_implementation());
        }
